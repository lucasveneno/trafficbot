const trafficBot = async id => {
	...
	let args = minimist(process.argv.slice(2), {
		...
		default: {
			url : 'https://lucasveneno.com/',
			help : 'Hello this is a traffic bot',
			version : '1.0'
		}
	});
	...
	*/
	...
	var proxy = "lucas:veneno@45.77.76.143:8080";// + port[Math.floor(Math.random() * (+ (port.length - 1) - + 0)) + + 0];

	const proxyNightmare = Nightmare({
		executionTimeout: 1000000,
		waitTimeout: 1000000, // in ms
		switches:{
			proxy: '45.77.76.143:8080', 
			proxyType: 'http', 
			proxyAuth : 'lucas:veneno'
		},
		//switches: {	'proxy-server': proxy, // set the proxy server here ...			'ignore-certificate-errors': true	},
		show: true 
	});

	var browsers = ['android-browser','chrome','firefox','internet-explorer','opera','safari'];
	var totalBrowsers = Object.keys(browsers).length -1;
	var browser = browsers[Math.floor(Math.random() * (+totalBrowsers - + 0)) + + 0];

	var userAgentObj = require("./useragent/"+browser+".json");
	var obj = JSON.parse(JSON.stringify(userAgentObj));

	var min = 0; 
	var max = Object.keys(obj).length - 1; 
	var random = Math.floor(Math.random() * (+max - +min)) + +min; 
	var ua = obj[random].ua;
	var gotourl, allLinks, addtourl;
	var utm_source = ['','facebook','twitter','instagram','Bing','Yahoo','DuckDuckGo','LinkedIn','Reddit','','Quora']
	var utm_sourcex = utm_source[(Math.floor(Math.random() * (+ (utm_source.length - 1) - + 0)) + + 0)];
	var utm_medium = ['cpc', 'Organic'];
	var utm_mediumx = utm_medium[(Math.floor(Math.random() * (+ (utm_medium.length - 1) - + 0)) + + 0)];
	var utm_term = ['queridin','queridin+music','música', 'online', 'ouvir', 'transmissão', 'tocar', 'digital', 'álbum', 'artista', 'playlist'];
	var utm_termx = utm_medium[(Math.floor(Math.random() * (+ (utm_term.length - 1) - + 0)) + + 0)];
